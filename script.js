...
const getWeatherData = async () => {
  ...
  try {
    ...
    const city = searchInput.value || 'Jabalpur'
...
const updateDom = data => {
  ...
  currentTemperature.innerText = ((data[0].main.temp.toFixed(1) -32 ) *(5/9)).toFixed(2)
  ...
  lowestToday.innerText = ((Math.round(data[0].main.temp_min) -32 )*(5/9)).toFixed(2)


  highestToday.innerText = ((Math.round(data[0].main.temp_max) -32)*(5/9)).toFixed(2)
    ...
    series: [
      {
        ...
        data: data.map(item => ((item.main.temp - 32) * (5/9)).toFixed(2)),
      },
    ],
  }
